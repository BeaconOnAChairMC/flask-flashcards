...
original_author="BeaconOnAChairMC"
original_name="flask_flashcards"
original_urlname="flask-flashcards"
original_description="Awesome flask_flashcards created by BeaconOnAChairMC" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_flashcards "${name}"
...
then
    ...
    echo "then run 'flask_flashcards' or 'python -m flask_flashcards'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_flashcards' or 'python -m flask_flashcards' or 'poetry run flask_flashcards'"
fi
