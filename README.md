...
- 🎯 Entry points to execute your program using `python -m <flask_flashcards>` or `$ flask_flashcards` with basic CLI argument parsing.
...
---
# flask_flashcards Flask Application

Awesome flask_flashcards created by BeaconOnAChairMC
...
```bash
git clone https://github.com/BeaconOnAChairMC/flask-flashcards flask_flashcards
cd flask_flashcards
make install
```
...
```bash
flask_flashcards create-db   # run once
flask_flashcards populate-db  # run once (optional)
flask_flashcards add-user -u admin -p 1234  # ads a user
flask_flashcards run
```
